...
def train(train_set,test_set,attributes,constant,n,threshold):
	...
	fn=1
	for i in range(len(test_set)):
		if test_data[i][44]=='Yes' and test_data[i][45]==1:
			acc+=1
			tp+=1
		elif test_data[i][44]=='No' and test_data[i][45]==0:
			tn+=1
			acc+=1
		elif test_data[i][44]=='Yes' and test_data[i][45]==0:
			fn+=1
		elif test_data[i][44]=='No' and test_data[i][45]==1:
			fp+=1
